...
public partial class tasksetadd : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
            ...
            comm.CommandText = "sp_insert_Task";
            comm.Connection = con;
            SqlParameter[] co = new SqlParameter[2];
            co[0] = new SqlParameter("@p_task_name", TextBox1.Text);
            co[1] = new SqlParameter("@p_crs_id", DropDownList1.SelectedValue);
            comm.Parameters.AddRange(co);
            con.Open();
            int x = comm.ExecuteNonQuery();
        
        if (x > -1)
            Label3.Text = x.ToString() + "affected rows";
        else
            Label3.Text = "no Data";
        con.Close();
